...
#define SIGFILTER(NAME) \
if(bFiltered == false && strncmp(szSig + 1, NAME, strlen(NAME)) == 0) \
{ \
   debug(DEBUGLEVEL_DEBUG, ", ##NAME"); \
   while(strchr("\r\n", *szSig) != NULL) \
   { \
      *szSig = '\0'; \
      szSig--; \
...
void CmdMessageView(EDF *pReply, int iFolderID, char *szFolderName, int iMsgNum, int iNumMsgs, bool bPage)
{
   ...
   if(pReply->GetChild("text", &szText) == true && szText != NULL)
   {
      ...
      if(bSigFilter == true)
      {
         szSig = szText + strlen(szText) - 1;
         debug(DEBUGLEVEL_DEBUG, "CmdMessageView sig point '%s'\n", szSig);
         while(szSig > szText && strchr("\r\n\t ", *szSig) != NULL)
         {
            szSig--;
            debug(DEBUGLEVEL_DEBUG, "CmdMessageView sig point 1 '%s'\n", szSig);
         }
         ...
         if(szSig > szText)
         {
            ...
            {
               szSig--;
               debug(DEBUGLEVEL_DEBUG, "CmdMessageView sig point 2 '%s'\n", szSig);
            }
