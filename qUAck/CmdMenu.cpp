...
void MatchAdd(EDF *pEDF, bool *bNot, char *szCommand, char *szValue)
{
   ...
   if(stricmp(szCommand, "and") == 0 || stricmp(szCommand, "&&") == 0 || stricmp(szCommand, "or") == 0 || stricmp(szCommand, "||") == 0)
   {
      if(stricmp(szCommand, "&&") == 0)
      {
         szTempCmd = "and";
      }
   ...
   else if(stricmp(szCommand, "not") == 0 || stricmp(szCommand, "!") == 0)
   {
      debug("MatchAdd not\n");
      (*bNot) = !*(bNot);
   }
   else if(stricmp(szCommand, "noparent") == 0)
   {
      pEDF->AddChild(szCommand);
   }
...
char *EDFToMatch(EDF *pEDF, int iIndent)
{
   ...
   if(stricmp(szName, "folderid") == 0)
   {
      ...
      {
         sprintf(szField, "folder:\0373%s%s%s\0370", strchr(szFolder, ' ') != NULL ? "\"" : "", szFolder, strchr(szFolder, ' ') != NULL ? "\"" : "");
         delete[] szFolder;
      }
   }
   else if(stricmp(szName, "fromid") == 0)
   {
      ...
      {
         sprintf(szField, "from:\0373%s%s%s\0370", strchr(szUser, ' ') != NULL ? "\"" : "", szUser, strchr(szUser, ' ') != NULL ? "\"" : "");
         delete[] szUser;
      }
   }
   else if(stricmp(szName, "toid") == 0)
   {
      if(UserGet(m_pUserList, lValue, &szUser, true) == true)
      {
         sprintf(szField, "to:\0373%s%s%s\0370", strchr(szUser, ' ') != NULL ? "\"" : "", szUser, strchr(szUser, ' ') != NULL ? "\"" : "");
         delete[] szUser;
      }
   }
   else if(stricmp(szName, "subject") == 0 || stricmp(szName, "text") == 0)
   {
      sprintf(szField, "%s:\0373%s%s%s\0370", szName, strchr(szValue, ' ') != NULL ? "\"" : "", szValue, strchr(szValue, ' ') != NULL ? "\"" : "");
   }
   ...
   {
      sprintf(szField, "noparent");
   }
   else
   {
      debug("EDFToMatch unmatched element %s %s %ld\n", szName, szValue, lValue);
   }
...
{
   STACKTRACE
   int iRuleID = 0, iMarkType = 0;
   bool bLoop = false, bStayCaughtUp = false;
   char cOption = '\0';
   char szWrite[100];
   char *szMatch = NULL;
   CmdInput *pInput = NULL;
   ...
   while(cOption != 'x')
   {
      ...
      if(m_pUser->Child("folders") == true)
      {
         ...
         while(bLoop == true)
         {
            ...
            m_pUser->GetChild("marktype", &iMarkType);

            sprintf(szWrite, "Rule \0373%d\0370: ", iRuleID);
            szMatch = EDFToMatch(m_pUser);
            CmdWrite(szWrite);
            CmdWrite(szMatch);
            if(iMarkType == THREAD_MSGCHILD)
            {
               CmdWrite(" (stay caught up)");
            }
            CmdWrite("\n");
      ...
      if(cOption == 'a')
      {
         bStayCaughtUp = CmdYesNo("Stay caught up on rule match", false);
         ...
         if(szMatch != NULL)
         {
            ...
            pRequest->Add("rule");
            pRequest->Copy(pMatch, false);
            if(bStayCaughtUp == true)
            {
               pRequest->AddChild("marktype", THREAD_MSGCHILD);
            }
            pRequest->Parent();
            pRequest->Parent();
            pRequest->Parent();
