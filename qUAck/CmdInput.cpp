...
#ifdef UNIX
      char szDir[200];
      sprintf(szDir, "%s/lib/aspell", SPELLROOT);
      SpellConfig("dict-dir", szDir);
      sprintf(szDir, "%s/share/aspell", SPELLROOT);
      SpellConfig("data-dir", szDir);
#endif
...
bool CmdInput::CheckWord(char *szWord)
{
   ...
   // Check the spelling
   debug("CmdInput::CheckWord %p '%s'\n", g_pSpeller, szWord);
   iCheck = aspell_speller_check(g_pSpeller, szWord, strlen(szWord));
   if(iCheck == 1)
   {
      return true;
   }
